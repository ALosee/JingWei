...
export async function generateModule(repositoryRoot: string, moduleId: string): Promise<void> {
  if (!/^[a-z][a-z0-9-]*$/u.test(moduleId))
    throw new Error('Usage: pnpm module:create <kebab-case-name>')
  const moduleRoot = join(repositoryRoot, 'packages/modules', moduleId)
  await mkdir(moduleRoot)
  await Promise.all(
    directories.map((directory) => mkdir(join(moduleRoot, directory), { recursive: true })),
  )
  await Promise.all(
    Object.entries(moduleFiles(moduleId)).map(([path, content]) =>
