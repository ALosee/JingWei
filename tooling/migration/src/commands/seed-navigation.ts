...
export async function runNavigationSeed(
  ...
  if (config.environment === 'production')
    throw new Error('Development navigation seed is forbidden in production')
  ...
  try {
    ...
    const users = await sql<{ id: string }>`SELECT id FROM iam."user"
      WHERE tenant_id = ${tenant.id} AND username_normalized = ${(environment.DEV_ADMIN_LOGIN ?? 'admin').toLowerCase()} AND status = 'ACTIVE'`.execute(
      db,
    )
    ...
    const roleId = await db.transaction().execute(async (tx) => {
      const result = await sql<{ id: string }>`INSERT INTO iam.role
        ...
        ON CONFLICT (tenant_id, code) DO UPDATE SET updated_at = iam.role.updated_at RETURNING id`.execute(
        tx,
      )
      ...
      await sql`INSERT INTO iam.user_role (tenant_id, user_id, role_id, created_at, created_by)
        VALUES (${tenant.id}, ${user.id}, ${role.id}, now(), ${user.id}) ON CONFLICT DO NOTHING`.execute(
        tx,
      )
      for (const module of generatedEdition.modules) {
        for (const permission of module.manifest.permissions) {
          ...
          await sql`INSERT INTO iam.role_permission (tenant_id, role_id, permission_code, scope_type, created_at, created_by)
            VALUES (${tenant.id}, ${role.id}, ${permission.code}, 'ALL', now(), ${user.id}) ON CONFLICT DO NOTHING`.execute(
            tx,
          )
        }
      }
      await new PostgresAuditWriter(tx).append({
        context,
        module: 'iam',
        action: 'development_role_initialized',
        entityType: 'role',
        entityId: role.id,
        result: 'SUCCESS',
    ...
    if (publishedId === null) {
      ...
      console.log(
        'Created and published initial database navigation revision ' + String(published.revision),
      )
    ...
    const assignable = published.nodes
      .filter((node) => node.accessMode === 'PERMISSION')
      .map((node) => node.code)
    const codes = [...new Set([...current.codes, ...assignable])].filter((code) =>
      assignable.includes(code),
    )
