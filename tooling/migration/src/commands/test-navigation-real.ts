...
export async function runNavigationIntegrationTests(
  environment: NodeJS.ProcessEnv = process.env,
): Promise<void> {
  const config = loadConfig(environment)
  if (config.environment === 'production')
    throw new Error('Real navigation tests are forbidden in production')
  ...
  try {
    ...
    const result = spawnSync(
      'pnpm',
      ['exec', 'vitest', 'run', 'tooling/migration/src/navigation.integration.test.ts'],
    ...
    if (created) {
      ...
      console.log(
        'Removed temporary test database ' + name + '; application database was not changed',
      )
    }
