...
import { navigationResponseSchema, versionSchema, type NavigationVersion } from '@jingwei/module-navigation/shared'
...
describe.skipIf(databaseUrl === undefined)('real PostgreSQL navigation/API', () => {
  ...
  const legacyTenant = newTenantId(), legacyRoot = newEntityId(), legacyVersion = newEntityId()
  beforeAll(async () => {
    if (databaseUrl === undefined || !new URL(databaseUrl).pathname.startsWith('/jingwei_navigation_test_')) {
      throw new Error('Use test:navigation:real; integration tests require a disposable database')
    }
    runtime = createRuntime({ NODE_ENV: 'test', DATABASE_URL: databaseUrl, APP_ORIGIN: 'http://localhost:5173' })
    const migrator = new Migrator({ db: runtime.database.view(), provider: new StaticMigrationProvider(generatedMigrations) })
    ...
    await sql`INSERT INTO navigation.navigation_version (id, tenant_id, navigation_id, version, status, published_at, created_at)
      VALUES (${legacyVersion}, ${legacyTenant}, ${legacyRoot}, 1, 'PUBLISHED', now(), now())`.execute(db)
    await sql`UPDATE navigation.navigation SET published_version_id = ${legacyVersion} WHERE id = ${legacyRoot}`.execute(db)
    await sql`INSERT INTO navigation.navigation_node (id, tenant_id, version_id, type, route_key, path, title, visible, access_mode)
      ...
      (${newEntityId()}, ${legacyTenant}, ${legacyVersion}, 'ROUTE', 'iam.account', '/account', 'Legacy account', true, 'AUTHENTICATED')`.execute(db)
    ...
    const silent = { child: () => silent, debug() { /* quiet test logger */ }, info() { /* quiet test logger */ },
      warn() { /* quiet test logger */ }, error() { /* failures asserted through HTTP */ } }
    app = await createApp({ ...runtime, logger: silent })
  })
  afterAll(async () => { await runtime.dispose() })

  async function request(path: string, session: CreatedSession | null, method = 'GET', body?: unknown, csrf = true) {
    const headers = new Headers({ origin: 'http://localhost:5173' })
    if (session) {
      headers.set('cookie', sessionCookieName + '=' + session.token + '; ' + csrfCookieName + '=' + session.csrfToken)
  ...
  async function fixture() {
    ...
    await sql`INSERT INTO platform.tenant (id, code, name, status, default_locale, default_timezone, default_currency, settings, created_at, updated_at)
      VALUES (${tenantId}, ${tenantCode}, 'Integration Tenant', 'ACTIVE', 'zh-CN', 'UTC', 'CNY', '{}', now(), now())`.execute(db)
    const makeUser = async (admin: boolean) => {
      const userId = newUserId(), roleId = newEntityId()
      await sql`INSERT INTO iam."user" (id, tenant_id, username, username_normalized, display_name, status, created_at, updated_at)
        VALUES (${userId}, ${tenantId}, ${userId}, ${userId}, 'Test User', 'ACTIVE', now(), now())`.execute(db)
      ...
      if (admin) {
        for (const module of generatedEdition.modules) for (const permission of module.manifest.permissions) {
    ...
    const admin = await makeUser(true), reader = await makeUser(false)
    ...
    const published = await request('/versions/' + draft.id + '/publish', admin.session, 'POST',
      { expectedEditRevision: 0, expectedPublishedVersionId: null })
    expect(published.status).toBe(200)
    return { tenantId, tenantCode, admin, reader, makeUser, published: versionSchema.parse(await published.json()) }
  }
  function saveBody(version: NavigationVersion) {
    return { authEntryCode: version.authEntryCode, homeCode: version.homeCode,
      nodes: version.nodes, expectedEditRevision: version.editRevision }
  }

  it('upgrades legacy ROUTE/visible data while preserving the published pointer', async () => {
    const db = runtime.database.view()
    const nodes = await sql<{ code: string; type: string; layout: string; name: string; params: unknown }>`
  ...
  it('loads database PUBLIC pages, enforces authentication/CSRF, and fails closed for missing tenants', async () => {
    ...
    expect((await request('/drafts', f.admin.session, 'POST', { sourceVersionId: null }, false)).status).toBe(403)
    ...
    const malformed = await app.request('/api/v1/navigation/drafts', {
      method: 'POST', body: '{', headers: {
        ...
        cookie: sessionCookieName + '=' + f.admin.session.token + '; ' + csrfCookieName + '=' + f.admin.session.csrfToken,
  ...
  it('saves defaults/layout in draft, rejects stale writes and keeps published versions immutable', async () => {
    const f = await fixture()
    const draft = versionSchema.parse(await (await request('/drafts', f.admin.session, 'POST', { sourceVersionId: null })).json())
    ...
    const savedResponse = await request('/versions/' + draft.id, f.admin.session, 'PUT', saveBody(draft))
    ...
    expect((await request('/versions/' + draft.id, f.admin.session, 'PUT', saveBody(draft))).status).toBe(409)
    expect((await request('/versions/' + f.published.id, f.admin.session, 'PUT', saveBody(f.published))).status).toBe(409)
    expect((await request('/versions/' + saved.id + '/publish', f.admin.session, 'POST',
      { expectedEditRevision: 1, expectedPublishedVersionId: f.published.id })).status).toBe(200)
    ...
    await expect(sql`UPDATE navigation.navigation_node SET name = 'illegal' WHERE version_id = ${saved.id}`.execute(runtime.database.view()))
      .rejects.toThrow('immutable')
  })

  it('serializes concurrent publication and supports rollback without editing snapshots', async () => {
    const f = await fixture()
    const drafts = await Promise.all([1, 2].map(async () => versionSchema.parse(
      await (await request('/drafts', f.admin.session, 'POST', { sourceVersionId: null })).json(),
    )))
    const responses = await Promise.all(drafts.map((draft) => request('/versions/' + draft.id + '/publish', f.admin.session, 'POST',
      { expectedEditRevision: 0, expectedPublishedVersionId: f.published.id })))
    expect(responses.map((r) => r.status).sort()).toEqual([200, 409])
    const current = navigationResponseSchema.parse(await (await request('/me', f.admin.session)).json())
    expect((await request('/versions/' + f.published.id + '/rollback', f.admin.session, 'POST',
      { expectedEditRevision: 0, expectedPublishedVersionId: current.versionId })).status).toBe(200)
    const rolledBack = navigationResponseSchema.parse(await (await request('/me', f.admin.session)).json())
  ...
  it('authorizes external links by navigation code, not routeKey, and never grants functional API rights', async () => {
    ...
    const account = draft.nodes.find((node) => node.code === 'iam.account')
    if (!account) throw new Error('fixture')
    draft.nodes.push({ ...account, id: newEntityId(), code: 'external.help', name: '外部帮助', type: 'EXTERNAL_LINK',
      routeKey: null, path: null, layout: null, href: 'https://example.com/help', externalTarget: 'BLANK', accessMode: 'PERMISSION' })
    const saved = versionSchema.parse(await (await request('/versions/' + draft.id, f.admin.session, 'PUT', saveBody(draft))).json())
    await request('/versions/' + saved.id + '/publish', f.admin.session, 'POST',
      { expectedEditRevision: saved.editRevision, expectedPublishedVersionId: f.published.id })
    const secondRole = await f.makeUser(false)
    await sql`INSERT INTO iam.user_role (tenant_id, user_id, role_id, created_at)
      VALUES (${f.tenantId}, ${f.reader.userId}, ${secondRole.roleId}, now())`.execute(runtime.database.view())
    expect((await request('/roles/' + secondRole.roleId + '/grants', f.admin.session, 'PUT',
      { codes: ['external.help'], expectedCodes: [] })).status).toBe(200)
    const granted = await request('/roles/' + f.reader.roleId + '/grants', f.admin.session, 'PUT',
      { codes: ['navigation.manage'], expectedCodes: [] })
    ...
    expect((await request('/admin', f.reader.session)).status).toBe(403)
    expect((await request('/roles/' + f.reader.roleId + '/grants', f.admin.session, 'PUT',
      { codes: [], expectedCodes: [] })).status).toBe(409)
    // Role status changes affect the next request; the session is not a stale permission cache.
    await sql`UPDATE iam.role SET status = 'DISABLED' WHERE id = ${f.reader.roleId}`.execute(runtime.database.view())
    ...
    expect((await request('/roles/' + secondRole.roleId + '/grants', f.admin.session, 'PUT',
      { codes: [], expectedCodes: ['external.help'] })).status).toBe(200)
    nav = navigationResponseSchema.parse(await (await request('/me', f.reader.session)).json())
  ...
  it('isolates tenant versions, role grants and authenticated bootstrap tenant selection', async () => {
    ...
    expect((await request('/versions/' + a.published.id, b.admin.session, 'PUT', saveBody(a.published))).status).toBe(404)
    expect((await request('/roles/' + a.reader.roleId + '/grants', b.admin.session, 'PUT',
      { codes: ['navigation.manage'], expectedCodes: [] })).status).toBe(404)
    const nav = navigationResponseSchema.parse(await (await request('/bootstrap?tenantCode=' + a.tenantCode, b.admin.session)).json())
  ...
  it('rolls back pointer, snapshot and audit when transactional outbox append fails', async () => {
    ...
    const draft = versionSchema.parse(await (await request('/drafts', f.admin.session, 'POST', { sourceVersionId: null })).json())
    const db = runtime.database.view()
    const auditCount = async () => (await sql<{ count: number }>`SELECT count(*)::int as count FROM platform.audit_log WHERE tenant_id = ${f.tenantId}`.execute(db)).rows[0]?.count
    const before = await auditCount()
    await sql`CREATE FUNCTION platform.test_reject_outbox() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected outbox failure'; END $$`.execute(db)
    await sql`CREATE TRIGGER test_reject_outbox BEFORE INSERT ON platform.outbox FOR EACH ROW EXECUTE FUNCTION platform.test_reject_outbox()`.execute(db)
    try {
      const response = await request('/versions/' + draft.id + '/publish', f.admin.session, 'POST',
        { expectedEditRevision: 0, expectedPublishedVersionId: f.published.id })
    ...
    expect(versionSchema.parse(await (await request('/versions/' + draft.id, f.admin.session)).json()).status).toBe('DRAFT')
  })
})
