...
export async function checkArchitecture(repositoryRoot: string): Promise<ArchitectureViolation[]> {
  ...
  const scripts = typeof rootPackage === 'object' && rootPackage !== null && 'scripts' in rootPackage ? rootPackage.scripts : null
  if (typeof scripts !== 'object' || scripts === null) throw new Error('Root package must declare scripts')
  const commands: unknown[] = Object.values(scripts)
  const commandEntrypoints = new Set(commands.filter((value): value is string => typeof value === 'string').flatMap((command) =>
    [...command.matchAll(/\b(tooling\/[\w/-]+\.ts)\b/gu)].map((match) => match[1]),
  ))
  const files = (await Promise.all(['apps', 'packages', 'tooling'].map((directory) =>
  ...
  for (const file of files) {
    ...
    const entrypoint = /^apps\/[^/]+\/src\/(?:main|index)\.ts$/u.test(label)
      || /^tooling\/[^/]+\/src\/cli\.ts$/u.test(label) || commandEntrypoints.has(label)
    violations.push(...inspectSourceResponsibilities(label, await readFile(file, 'utf8'), { entrypoint }))
  }
  return violations
}
