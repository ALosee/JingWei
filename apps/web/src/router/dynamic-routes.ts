...
const definitions = new Map(
  generatedWebModules.flatMap(({ manifest }) =>
    manifest.routeDefinitions.map((d) => [d.key, d] as const),
  ),
)
const bindings = new Map(
  generatedWebModules.flatMap(({ pages }) => pages.map((p) => [p.routeKey, p.pageKey] as const)),
)
...
export function installDynamicRoutes(router: Router, navigation: NavigationResponse): void {
  ...
  for (const node of navigation.nodes) {
    ...
    if (
      ...
      !definition.allowedAccessModes.includes(node.accessMode) ||
