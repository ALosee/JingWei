...
2. 并行请求 `GET /api/v1/branding/bootstrap`；成功时注册内容寻址的租户自定义色阶，并将命名方案与
   明暗语义覆盖更新到登录页、document、租户主题和工作区默认值；失败时继续使用内置默认值。
...
10. 会话恢复后以 tenantId/userId 加载浏览器本地的明暗/尺寸偏好和稀疏布局覆盖；未覆盖布局字段继续跟随生效租户默认值。
11. 任一 Registry/Navigation 错误都跳转 `/__recovery`；品牌请求失败不阻断认证或恢复页。
