...
首次启用 Outbox 时，worker 的停止领取、等待当前分发和释放资源必须加入同一个有界关闭流程。
...
当前没有事件生产者、dispatcher 或 worker，因此不能把 Outbox 计入已运行组件，也没有 worker 心跳或投递成功率指标。数据库中若存在启用前遗留记录，先按事件类型、版本和预期消费者分类，不得直接删除或修改 `published_at`。

Outbox 正式启用后，至少监控未发布数量、最老事件年龄、成功/失败率、连续重试、分发延迟和 worker 心跳；告警优先基于用户影响和积压年龄，而不是单次瞬时失败。
...
当前版本不会处理 Outbox；出现记录时先确认它们是否为启用前的历史写入，并按 ADR 0014 分类，不能假设重启 Server 会自动投递。正式启用后再检查 dispatcher 下游、数据库锁/连接、worker 日志与重试次数；不要直接把事件标成成功。
