...
- 一个业务事务中的状态和审计使用同一连接；已启用事件的 outbox append 也必须使用该连接。
...
2. 已有具名真实消费者时发布 Integration Event，并由已接线的 outbox 保证与业务事务一致；
3. 把真正通用、无业务所有者的技术机制放到 platform。

不要为“未来可能需要”创建事件或接入 Noop Dispatcher。新增事件必须同时说明消费者、版本、幂等、失败恢复和运行时生命周期，具体门槛见 ADR 0014。
