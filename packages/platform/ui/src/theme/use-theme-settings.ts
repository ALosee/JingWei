...
export function useThemeSettings(options: UseThemeSettingsOptions = {}): UseThemeSettingsReturn {
  ...
  const setOverride = (mode: 'light' | 'dark', key: ColorKey, value: ColorValue | ''): void => {
    ...
    const modeTokens: Record<string, ColorValue> = { ...current[mode] }
    const nextModeTokens =
      value === '' || !isValidColorValue(value)
        ? Object.fromEntries(Object.entries(modeTokens).filter(([token]) => token !== key))
        : { ...modeTokens, [key]: value }

    const next: ThemeOverrides = { ...current, [mode]: nextModeTokens }

    if (!hasAnyOverride(next)) {
      const nextState = { ...state.value }
      Reflect.deleteProperty(nextState, 'overrides')
  ...
  const resolved = computed<ThemeOptions>(() => {
    const override = state.value.overrides
    const rest = { ...state.value }
    Reflect.deleteProperty(rest, 'mode')
    Reflect.deleteProperty(rest, 'overrides')
