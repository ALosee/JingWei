...
import {
  ...
  simpleCssColor,
  toCssColor
} from './palette-picker-shared';
import type { PaletteChangePayload, PalettePickerEmits, PalettePickerProps, PaletteSelectValue } from './palette-picker-types';
...
// —— custom 面板内部状态 ——
const customColor = ref<string>(isCustom.value ? (typeof model.value === 'string' ? model.value : typeof model.value === 'string' ? model.value : String(model.value)) : '#6366f1');
...
const triggerLabel = computed(() => {
  if (isCustom.value) {
    return typeof model.value === 'string' ? model.value : typeof model.value === 'string' ? model.value : String(model.value);
  }
...
const onToggleCustom = (isOn: boolean): void => {
  ...
  customLevel.value = deriveNearestLevel(typeof model.value === 'string' ? model.value : String(model.value), props.format);
