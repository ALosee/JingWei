...
const lastKey = ref<TailwindPaletteKey | SimplePaletteKey>(
  isTailwindKey(initialSelectValue) || isSimpleKey(initialSelectValue)
    ? initialSelectValue
    : 'blue',
)
...
const nearest = computed(() =>
  recommended.value ? generateNearestPalette(customColor.value, 'oklchString') : null,
)
...
const tailwindPaletteLevels = computed<Partial<Record<PaletteColorLevel, string>>>(() => {
  ...
  return Object.fromEntries(
    PALETTE_LEVELS.map((level) => [level, tailwindPalette[key][level][itemColorKey.value]]),
  )
})
...
const onToggleCustom = (isOn: boolean): void => {
  ...
  customLevel.value = deriveNearestLevel(
    typeof model.value === 'string' ? model.value : String(model.value),
    props.format,
  )
      ...
      <div v-if="isCustom" class="space-y-1.5 border-t border-border px-2 py-1.5">
        ...
        <div class="flex items-center justify-between gap-2">
          <span class="text-xs text-muted-foreground">{{
            palettePickerMessages.recommendedPalette
          }}</span>
