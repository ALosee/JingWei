...
import type { PaletteSelectValue } from './palette-picker-types'
 ...
 */
export const TAILWIND_KEYS: readonly TailwindPaletteKey[] = Object.keys(
  tailwindPalette,
) as TailwindPaletteKey[]
...
export const isTailwindKey = (value: string): value is TailwindPaletteKey =>
  value in tailwindPalette
 ...
 */
export function parseTailwindValue(
  value: string,
): { key: TailwindPaletteKey; level: PaletteColorLevel } | null {
  const [key, level] = value.split('.')

  if (key === undefined || !isTailwindKey(key)) {
    return null
  }
...
export function deriveTailLevel(value: ColorValue): PaletteColorLevel {
  return (
    parseTailwindValue(typeof value === 'string' ? value : String(value))?.level ?? DEFAULT_LEVEL
  )
}
...
export function deriveNearestLevel(color: string, format: ColorFormat): PaletteColorLevel {
  ...
  try {
    ...
    return generateNearestPalette(color, outputFormatOf(format)).level
  } catch {
    return DEFAULT_LEVEL
  }
}
...
export function serializeColor(color: string, format: ColorFormat): ColorValue {
  return (
    format === 'oklch' ? colord(color).toOklchString() : colord(color).toHslString()
  ) as ColorValue
}
 ...
 */
export const itemColorKeyOf = (format: ColorFormat): 'oklch' | 'hsl' =>
