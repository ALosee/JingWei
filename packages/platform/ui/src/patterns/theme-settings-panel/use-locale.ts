...
export function useThemeCustomizerLocale() {
  ...
  const resolveLabel = (key: string): string => {
    ...
    if (key.startsWith('theme.group.')) {
      return m.groups[key.slice('theme.group.'.length) as keyof typeof m.groups];
    }

    if (key.startsWith('theme.variant.')) {
      return m.variants[key.slice('theme.variant.'.length) as keyof typeof m.variants];
    }

    return m.sections[key as keyof typeof m.sections];
  };
