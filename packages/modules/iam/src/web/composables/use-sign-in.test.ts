...
it('enters the workspace only after login completes', async () => {
  const login = vi.fn(() => Promise.resolve({ user: { id: 'user', tenantId: 'tenant', displayName: 'Test' }, csrfToken: 'test' }))
  ...
  await form.submit()
  expect(login).toHaveBeenCalledWith({ tenantCode: 'default', login: 'admin', password: 'test-password' })
...
it('retains a readable error and resets submitting on authentication failure', async () => {
  const enterWorkspace = vi.fn()
  const form = useSignIn({ login: () => Promise.reject(new Error('Invalid credentials')), enterWorkspace })
  await form.submit()
