...
import {
  PostgresNavigationStore,
  PostgresNavigationUnitOfWork,
  type NavigationDatabase,
} from '../infrastructure/navigation-store.pg.js'
...
export function createNavigationManagement(database: DatabaseRuntime, registry: ModuleRegistry) {
  ...
  return new ManageNavigation(
    new PostgresNavigationStore(view),
    new PostgresNavigationUnitOfWork(view),
    registry,
