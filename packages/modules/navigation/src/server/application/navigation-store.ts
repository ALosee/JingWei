...
import type { AdminNavigation, NavigationConfiguration, NavigationVersion } from '../../shared/index.js'
import type { NavigationSource } from '../domain/navigation.js'

export interface NavigationRoot { id: string; publishedVersionId: string | null }
export interface NavigationStore extends NavigationSource {
  ...
  insertVersion(context: ApplicationContext, rootId: string, version: NavigationVersion): Promise<void>
  saveDraft(context: ApplicationContext, id: string, config: NavigationConfiguration, editRevision: number): Promise<void>
  ...
  replaceRoleCodes(context: ApplicationContext, roleId: string, codes: readonly string[]): Promise<void>
}
export interface NavigationTransaction {
  store: NavigationStore
  record(context: ApplicationContext, action: string, entityId: string, before: unknown, after: unknown): Promise<void>
}
