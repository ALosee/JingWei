...
export function validateNavigation(
  ...
  const paths: string[][] = []
  const add = (code: string, nodeId: string, message: string) => {
    issues.push({ code, nodeId, message })
  }
  if (byId.size !== config.nodes.length)
    issues.push({ code: 'NAVIGATION_ID_DUPLICATE', message: '节点 ID 重复' })
  for (const node of config.nodes) {
    ...
    while (ancestor !== null) {
      if (seen.has(ancestor)) {
        add('NAVIGATION_CYCLE', node.id, '父链存在循环')
        break
      }
      seen.add(ancestor)
    ...
    if (isContainer(node)) {
      if (
        ...
        Object.keys(node.query).length > 0
      ) {
        add(
          'NAVIGATION_CONTAINER_TARGET',
          node.id,
          '目录/分组不能设置路由、参数、外链或独立访问模式',
        )
      }
      continue
    }
    if (node.accessMode === null)
      add('NAVIGATION_ACCESS_MODE_REQUIRED', node.id, '必须选择访问模式')
    ...
    if (node.href !== null || node.externalTarget !== null)
      add('NAVIGATION_INTERNAL_FIELDS', node.id, '内部页面不能设置外链')
    ...
    if (definition === null) add('NAVIGATION_ROUTE_UNKNOWN', node.id, 'routeKey 不在当前 Edition')
      ...
      if (routes.has(definition.key))
        add('NAVIGATION_ROUTE_DUPLICATE', node.id, '一个版本只能挂载一次 routeKey')
      ...
      if (
        ...
        !registry.hasCapability(definition.requiredCapability)
      ) {
        add('NAVIGATION_CAPABILITY_DISABLED', node.id, '页面能力未启用')
      }
      if (
        definition.requiredPermission !== undefined &&
        registry.permission(definition.requiredPermission) === null
      ) {
    ...
    if (parameters === null || node.path?.startsWith('/__')) {
      add(
        ...
        node.id,
        '路径必须为非保留绝对路径；只允许普通参数和末尾可选参数',
      )
      continue
    }
    const segments = (node.path ?? '').toLowerCase().slice(1).split('/')
    const alternatives = segments.at(-1)?.endsWith('?')
    ...
    for (const key of Object.keys(node.params)) {
      if (!parameters.some((parameter) => parameter.name === key))
        add('NAVIGATION_PARAM_UNKNOWN', node.id, '默认参数不在路径中: ' + key)
      if (node.params[key] === '.' || node.params[key] === '..')
        add('NAVIGATION_PARAM_INVALID', node.id, '默认参数不能是路径跳转片段: ' + key)
    }
    if (node.type === 'MENU' && navigationTarget(node) === null)
      add('NAVIGATION_PARAMS_REQUIRED', node.id, '菜单必须提供全部必填路径参数')
  }
  for (const [code, isAuth] of [
    [config.authEntryCode, true],
    [config.homeCode, false],
  ] as const) {
    ...
    if (
      ...
      !isInternal(node) ||
      !enabled ||
      navigationTarget(node) === null ||
      (isAuth && node.accessMode !== 'PUBLIC')
    ) {
      issues.push({
        code: isAuth ? 'NAVIGATION_AUTH_ENTRY_INVALID' : 'NAVIGATION_HOME_INVALID',
