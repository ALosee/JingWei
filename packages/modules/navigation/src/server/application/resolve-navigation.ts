...
export function projectNodes(nodes: readonly NavigationNode[], authenticated: boolean, codes: ReadonlySet<string>): NavigationNode[] {
  ...
  const allowed = nodes.filter((node) => !isContainer(node) && enabled(node)
    && (node.accessMode === 'PUBLIC' || (authenticated
      && (node.accessMode === 'AUTHENTICATED' || (node.accessMode === 'PERMISSION' && codes.has(node.code))))))
  ...
  return nodes.filter((node) => kept.has(node.id)).map((node) => {
    let parentId = node.parentId
    while (parentId !== null && !kept.has(parentId)) parentId = byId.get(parentId)?.parentId ?? null
    return { ...node, parentId }
  }).sort((a, b) => a.sortOrder - b.sortOrder || a.code.localeCompare(b.code))
}

export class ResolveNavigation {
  constructor(private readonly source: NavigationSource, private readonly registry: ModuleRegistry,
  ...
  bootstrap(tenantId: TenantId): Promise<NavigationResponse> { return this.resolve(tenantId, null) }
  /** Reads live role grants on every call. AuthContext.roleIds is not an authorization cache. */
  forUser(context: AuthContext): Promise<NavigationResponse> { return this.resolve(context.tenantId, context) }

  private async resolve(tenantId: TenantId, context: AuthContext | null): Promise<NavigationResponse> {
    const version = await this.source.loadPublished(tenantId)
    if (version === null) throw new ApplicationError({ code: 'NAVIGATION_NOT_PUBLISHED', message: '当前租户尚未发布导航', status: 503 })
    if (validateNavigation(version, this.registry).length > 0) {
      throw new ApplicationError({ code: 'NAVIGATION_CONFIGURATION_INVALID', message: '已发布导航与当前 Edition 不兼容，请联系管理员', status: 503 })
    }
    ...
    return { schemaVersion: 2, versionId: version.id, publishedRevision: version.revision,
      authEntryCode: version.authEntryCode,
      homeCode: nodes.some((node) => node.code === version.homeCode) ? version.homeCode : null, nodes }
  }
}
