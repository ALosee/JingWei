...
import { isContainer, type NavigationConfiguration, type NavigationVersion, type PublishNavigation,
  type SaveDraft, type SaveRoleGrants } from '../../shared/index.js'
...
function fail(code: string, message: string, status = 409): never { throw new ApplicationError({ code, message, status }) }
...
export class ManageNavigation {
  constructor(private readonly store: NavigationStore, private readonly work: NavigationUnitOfWork,
    private readonly registry: ModuleRegistry, private readonly access: IamAccess) {}
  ...
  async catalog(context: AuthContext) {
    await this.authorize(context, 'view')
    return { routes: this.registry.routes().map((route) => ({
      key: route.key, layout: route.layout, allowedLayouts: [...(route.allowedLayouts ?? [route.layout])],
  ...
  async createDraft(context: AuthContext, sourceId: string | null) {
    ...
    return this.work.run(async (tx) => {
      ...
      const config = source === null ? createDefaultConfiguration(this.registry) : await this.requiredVersion(tx.store, context, source)
      const version: NavigationVersion = {
        ...cloneNodes(config), id: newEntityId(), revision: await tx.store.nextRevision(context.tenantId),
        editRevision: 0, status: 'DRAFT', publishedAt: null,
      }
      await tx.store.insertVersion(context, root.id, version)
      await tx.record(context, 'draft_created', version.id, null, { revision: version.revision, sourceId: source })
      return version
    })
  }
  ...
  async save(context: AuthContext, id: string, input: SaveDraft) {
    ...
    return this.work.run(async (tx) => {
      ...
      if (version.editRevision !== input.expectedEditRevision) fail('NAVIGATION_EDIT_CONFLICT', '草稿已被其他人修改，请重新加载')
      const config = cloneNodes(input)
      await tx.store.saveDraft(context, id, config, version.editRevision + 1)
      await tx.record(context, 'draft_saved', id, { editRevision: version.editRevision },
        { editRevision: version.editRevision + 1, nodeCodes: config.nodes.map((node) => node.code) })
  ...
  async publish(context: AuthContext, id: string, input: PublishNavigation, rollback = false) {
    ...
    return this.work.run(async (tx) => {
      ...
      if (root.publishedVersionId !== input.expectedPublishedVersionId) fail('NAVIGATION_PUBLISH_CONFLICT', '发布指针已经变化，请重新加载')
      const version = await this.requiredVersion(tx.store, context, id)
      if (version.editRevision !== input.expectedEditRevision) fail('NAVIGATION_EDIT_CONFLICT', '版本已变化，请重新加载')
      ...
      await tx.record(context, rollback ? 'rolled_back' : 'published', root.id,
        { versionId: root.publishedVersionId }, { versionId: id, revision: version.revision })
      return this.requiredVersion(tx.store, context, id)
    })
  }
  ...
  async grantRole(context: AuthContext, roleId: string, input: SaveRoleGrants) {
    ...
    return this.work.run(async (tx) => {
      const root = await tx.store.root(context.tenantId, true)
      if (root?.publishedVersionId === null || root === null) fail('NAVIGATION_NOT_PUBLISHED', '请先发布导航', 422)
      ...
      const assignable = new Set(version.nodes.filter((node) => !isContainer(node) && node.accessMode === 'PERMISSION').map((node) => node.code))
      const codes = [...new Set(input.codes)].sort()
      if (codes.some((code) => !assignable.has(code))) fail('NAVIGATION_GRANT_INVALID', '只能授权当前发布版本的 PERMISSION 节点 code', 422)
  ...
  private assertValid(config: NavigationConfiguration) {
    ...
    if (issues.length > 0) throw new ApplicationError({
      code: 'NAVIGATION_VALIDATION_FAILED', message: '导航配置校验失败', status: 422, details: { issues },
    })
  }
}
...
function cloneNodes(config: NavigationConfiguration): NavigationConfiguration {
  ...
  return { authEntryCode: config.authEntryCode, homeCode: config.homeCode, nodes: config.nodes.map((node) => {
    const id = ids.get(node.id)
    if (id === undefined) throw new Error('Node clone invariant')
    return { ...node, id, parentId: node.parentId === null ? null : ids.get(node.parentId) ?? null }
  }) }
}
