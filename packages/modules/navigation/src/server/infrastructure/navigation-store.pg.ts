...
} from '../application/navigation-store.js'
...
interface NodeRow {
  ...
  path: string | null
  layout: NavigationNode['layout']
  icon: string | null
  sort_order: number
  access_mode: NavigationNode['accessMode']
  href: string | null
  external_target: NavigationNode['externalTarget']
  params: unknown
  query: unknown
}
...
export class PostgresNavigationStore implements NavigationStore {
  ...
  async root(tenantId: TenantId, lock = false) {
    let query = this.db
      .selectFrom('navigation.navigation')
  ...
  async ensureRoot(context: ApplicationContext) {
    const now = new Date()
    await this.db
      .insertInto('navigation.navigation')
      .values({
        id: newEntityId(),
      ...
      .onConflict((conflict) => conflict.columns(['tenant_id', 'code']).doNothing())
      .execute()
  ...
  async list(tenantId: TenantId) {
    ...
    const rows = await this.db
      .selectFrom('navigation.navigation_version')
      .selectAll()
      .where('tenant_id', '=', tenantId)
      .where('navigation_id', '=', root.id)
    ...
    return {
      ...
      versions: rows.map((row) => ({
        ...
        authEntryCode: row.auth_entry_code,
  ...
  async version(tenantId: TenantId, id: string): Promise<NavigationVersion | null> {
    ...
    const nodes = await this.db
      .selectFrom('navigation.navigation_node')
      .selectAll()
      .where('tenant_id', '=', tenantId)
      .where('version_id', '=', id)
      .orderBy('sort_order')
    ...
    return versionSchema.parse({
      ...
      nodes: nodes.map((n) => ({
        ...
        href: n.href,
        externalTarget: n.external_target,
        params: n.params,
        query: n.query,
      })),
    })
  ...
  async nextRevision(tenantId: TenantId) {
    const result = await this.db
      .selectFrom('navigation.navigation_version')
      .select((eb) => eb.fn.max('version').as('maximum'))
      .where('tenant_id', '=', tenantId)
      .executeTakeFirst()
  ...
  async insertVersion(context: ApplicationContext, rootId: string, version: NavigationVersion) {
    await this.db
      .insertInto('navigation.navigation_version')
      .values({
        id: version.id,
        tenant_id: context.tenantId,
        navigation_id: rootId,
        ...
        created_by: context.userId,
        published_by: null,
        auth_entry_code: version.authEntryCode,
        home_code: version.homeCode,
        edit_revision: 0,
      })
      .execute()
  ...
  async saveDraft(
    ...
    await this.db
      .deleteFrom('navigation.navigation_node')
      .where('tenant_id', '=', context.tenantId)
      .where('version_id', '=', id)
      .execute()
    await this.insertNodes(context.tenantId, id, config.nodes)
    await this.db
      .updateTable('navigation.navigation_version')
      .set({
        auth_entry_code: config.authEntryCode,
        home_code: config.homeCode,
  ...
  private async insertNodes(
    ...
    await this.db
      .insertInto('navigation.navigation_node')
      .values(
        nodes.map((n) => ({
          id: n.id,
          tenant_id: tenantId,
          ...
          access_mode: n.accessMode,
          href: n.href,
          external_target: n.externalTarget,
          params: JSON.stringify(n.params),
          query: JSON.stringify(n.query),
        })),
      )
      .execute()
  }
  ...
  async pointPublished(context: ApplicationContext, rootId: string, versionId: string) {
    await this.db
      .updateTable('navigation.navigation')
      .set({
  ...
  async grantedCodes(tenantId: TenantId, roleIds: readonly string[]): Promise<ReadonlySet<string>> {
    ...
    const rows = await this.db
      .selectFrom('navigation.role_navigation')
      .select('navigation_code')
      .where('tenant_id', '=', tenantId)
  ...
  async replaceRoleCodes(context: ApplicationContext, roleId: string, codes: readonly string[]) {
    await this.db
      .deleteFrom('navigation.role_navigation')
      .where('tenant_id', '=', context.tenantId)
      .where('role_id', '=', roleId)
      .execute()
    if (codes.length > 0)
      await this.db
        .insertInto('navigation.role_navigation')
        .values(
          codes.map((code) => ({
            tenant_id: context.tenantId,
            role_id: roleId,
            navigation_code: code,
            created_at: new Date(),
            created_by: context.userId,
          })),
        )
