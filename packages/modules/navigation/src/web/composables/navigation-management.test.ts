...
function snapshot(): NavigationVersion {
  return {
    id: 'version-1', revision: 1, editRevision: 0, status: 'DRAFT', publishedAt: null,
    authEntryCode: 'signin', homeCode: null, nodes: [{
      id: 'node-1', code: 'signin', name: 'Sign in', parentId: null, type: 'PAGE', status: 'ENABLED',
      routeKey: 'iam.login', path: '/signin', layout: 'blank', icon: null, sortOrder: 0,
      accessMode: 'PUBLIC', href: null, externalTarget: null, params: {}, query: {},
    }]
  }
}
...
it('saves the expected revision, adopts returned node IDs, and preserves publish concurrency inputs', async () => {
  ...
  const saved = { ...draft, editRevision: 1, nodes: draft.nodes.map((node) => ({ ...node, id: 'saved-node' })) }
  const api = {
    getNavigationAdmin: vi.fn(() => Promise.resolve({ publishedVersionId: 'old-published', versions: [] })),
    getNavigationVersion: vi.fn(() => Promise.resolve({ ...draft, id: 'old-published', status: 'PUBLISHED' as const })),
    createNavigationDraft: vi.fn(() => Promise.resolve(draft)), saveNavigationDraft: vi.fn(() => Promise.resolve(saved)),
    validateNavigationVersion: vi.fn(() => Promise.resolve({ issues: [] })),
    publishNavigationVersion: vi.fn(() => Promise.resolve({ ...saved, status: 'PUBLISHED' as const })),
  }
  const versions = useNavigationVersions(api, editor, feedback, () => true)
  await versions.save()
  expect(api.saveNavigationDraft).toHaveBeenCalledWith(draft.id, expect.objectContaining({ expectedEditRevision: 0 }))
  expect(editor.selectedId.value).toBe('saved-node')
  await versions.publish(false)
  expect(api.publishNavigationVersion).toHaveBeenCalledWith(draft.id,
    { expectedEditRevision: 1, expectedPublishedVersionId: 'old-published' }, false)
})
...
it('uses the original role code set for compare-and-replace and honors confirmation', async () => {
  ...
  const api = {
    ...
    saveRoleNavigation: vi.fn((_role: string, input: SaveRoleGrants) => Promise.resolve({ codes: input.codes }))
  }
  ...
  await roles.saveGrants()
  expect(api.saveRoleNavigation).toHaveBeenCalledWith('role', { codes: ['new'], expectedCodes: ['old'] })
})
