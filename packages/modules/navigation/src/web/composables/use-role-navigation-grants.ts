...
export function useRoleNavigationGrants(api: RoleClient, published: Ref<NavigationVersion | null>,
  ...
  const assignable = computed(() => published.value?.nodes.filter((node) => !isContainer(node) && node.accessMode === 'PERMISSION') ?? [])
  const retiredCodes = computed(() => grants.value.filter((code) => !assignable.value.some((node) => node.code === code)))
  async function loadRole() {
    await run(async () => {
      // Failed reads must not leave another role's editable grants on screen.
      loadedRoleId.value = ''; grants.value = []; originalGrants.value = []
      if (!roleId.value) return
      const result = await getRoleNavigation(roleId.value)
      grants.value = [...result.codes]; originalGrants.value = [...result.codes]
  ...
  async function saveGrants() {
    ...
    await run(async () => {
      const result = await saveRoleNavigation(roleId.value, { codes: grants.value, expectedCodes: originalGrants.value })
      grants.value = [...result.codes]; originalGrants.value = [...result.codes]
