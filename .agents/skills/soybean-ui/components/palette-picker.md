...
Category: Forms
Description: `SPalettePicker` is a color picker that produces a full palette from a single base color. Its `modelValue` is a [`ColorValue`](https://ui.soybeanjs.cn/api/theme.md) — either a Tailwind palette key + level (e.g. `indigo.500`), a simple palette key (`black` / `white` / `transparent` …), or a raw custom color (`hsl(...)` / `oklch(...)`).

## Overview

`SPalettePicker` is a color picker that produces a full palette from a single base color. Its `modelValue` is a [`ColorValue`](https://ui.soybeanjs.cn/api/theme.md) — either a Tailwind palette key + level (e.g. `indigo.500`), a simple palette key (`black` / `white` / `transparent` …), or a raw custom color (`hsl(...)` / `oklch(...)`).
